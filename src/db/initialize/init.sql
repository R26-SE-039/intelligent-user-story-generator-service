...
-- Table: UTTERANCE_EMBEDDINGS
CREATE TABLE utterance_embeddings (
    utterance_id UUID PRIMARY KEY REFERENCES transcript_utterances(id) ON DELETE CASCADE,
    embedding vector(3072)
);

-- Table: REQUIREMENT_THREADS
...
CREATE TABLE requirements (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    meeting_id UUID REFERENCES meetings(id) ON DELETE CASCADE,
    thread_id UUID REFERENCES requirement_threads(id) ON DELETE SET NULL,
...
CREATE TABLE conflicts (
    ...
    previous_text_a TEXT,
    previous_text_b TEXT,
    resolved_by UUID,
...
-- Table: USER_STORY_VALIDATIONS
CREATE TABLE user_story_validations (
    id                    UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
